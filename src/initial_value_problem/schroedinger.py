...
def build_time_step_nls(stiffness: NDArray, mass: NDArray, dt: float = 0.1, param_p: int = 3, param_lambda: float = 1.0) -> Callable:

    def time_step_phi(u: NDArray, phi: NDArray) -> NDArray:
        return 2*np.abs(u)**(param_p-1) - phi

    def time_step_u(u_last: NDArray, phi: NDArray) -> NDArray:
    ...
    def time_step(u: NDArray, phi: NDArray) -> tuple[NDArray, NDArray]:
